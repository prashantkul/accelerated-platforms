...
gcloud auth configure-docker \
  "${FLOWER_EXAMPLE_CONTAINER_IMAGE_REPOSITORY_HOSTNAME}"

docker image push "${FLOWER_EXAMPLE_CONTAINER_IMAGE_LOCALIZED_ID_WITH_TAG}"
...
echo "Total runtime (Federated learning FLOWER example container image push): $(date -d@${total_runtime_value_federated_learning} -u +%H:%M:%S)"
